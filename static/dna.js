...
DNA.prototype.shuffle = function() {
  var i = Math.floor(random(this.order.length));
  var j = Math.floor(random(this.order.length));
  swap(this.order, i, j);
}
...
DNA.prototype.show = function() {
  ...
  for (var i = 0; i < this.order.length; i++) {
    ...
    text(cities[n].poiName,cities[n].x, cities[n].y)
  }
}
...
DNA.prototype.crossover = function(other) {
  ...
  var start = Math.floor(random(order1.length));
  var end = Math.floor(random(start + 1, order1.length + 1));
